...
class ImageSegmentationApp(QMainWindow):
    ...
    def save_masks(self) -> None:
        ...
        for entry in self.undo_stack:
            ...
            if entry.get('type') == 'flood_fill' and entry['class_idx'] != 0:
                mask.paste(entry['class_idx'], mask=entry['mask'])
    ...
    def finish_eraser(self) -> None:
        ...
        for entry in self.undo_stack:
            ...
            #         updated_mask = ImageChops.difference(fill_mask, eraser_mask)
            #         if updated_mask != fill_mask:
            #             # Update the entry if mask changed
            #             entry['mask'] = updated_mask
            #             # Recreate overlay
            #             color_layer = Image.new("RGBA", self.image.size, entry['color'].name())
            #             overlay = Image.composite(color_layer, Image.new("RGBA", self.image.size), updated_mask)
            #             qimage = self.pil2qimage(overlay)
            #             entry['overlay'].setPixmap(QPixmap.fromImage(qimage))
            # new_undo_stack.append(entry)
            if entry.get('type') == 'flood_fill':
        ...
        # Cleanup eraser visuals
        if self.eraser_path_item:
            self.scene.removeItem(self.eraser_path_item)
        self.eraser_path_item = None
        self.eraser_drawing = False
        self.eraser_path = None
        self.statusBar.showMessage("Erasing complete", 2000)
    ...
    def perform_flood_fill(self, x: int, y: int):
        ...
        self.undo_stack.append({
            ...
            'class_idx': self.class_colors.get(
