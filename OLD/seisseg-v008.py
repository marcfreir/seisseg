...
class ImageSegmentationApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # Eraser mode flag (initially off)
        self.eraser_mode = False

        # Add eraser action with a keyboard shortcut (e.g., "E")
        ...
        # Optionally, add an eraser button to your button_layout
    ...
    def toggle_eraser_mode(self) -> None:
        ...
        if self.eraser_mode:
            ...
            # Change cursor to indicate eraser mode (use an appropriate cursor)
    ...
    def handle_left_press(self, scene_pos: QPoint) -> None:
        ...
        # If eraser mode is active, perform erasing instead of drawing.
        if self.eraser_mode:
            self.handle_erase(scene_pos)
            return
    ...
    def handle_erase(self, scene_pos: QPoint) -> None:
        # Get the list of items at the clicked position.
        # The items() method returns a list with the topmost item first.
        items = self.scene.items(scene_pos)
        erased = False

        # Loop through the items and see if one belongs to a label.
        # We use the undo_stack as our record of label entries.
        for item in items:
            for entry in self.undo_stack:
                # Check if this item is either the overlay or the drawn path.
                if entry.get('overlay') == item or entry.get('path') == item:
                    ...
                    # Also remove it from self.labels if you are keeping that list.
                    try:
        ...
        if erased:
            self.statusBar.showMessage("Erased label", 2000)
        else:
            self.statusBar.showMessage("No label found to erase", 2000)
