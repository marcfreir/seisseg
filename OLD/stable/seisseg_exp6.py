...
class CustomGraphicsView(QGraphicsView):
    ...
    def wheelEvent(self, event: QEvent)  -> None:
        ...
        if not self.main_window.image:  # Access through main window reference
            return
...
class ImageSegmentationApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.eraser_button.clicked.connect(lambda: self.toggle_eraser_mode())
        ...
        self.auto_pick_button.clicked.connect(lambda: self.toggle_auto_pick_mode())
        ...
        self.flood_fill_button.clicked.connect(lambda: self.toggle_flood_fill_mode())
    ...
    def deactivate_other_modes(self, active_mode: str) -> None:
        """Deactivate all modes except the specified one"""
        modes = {
            'eraser': self.toggle_eraser_mode,
            'auto_pick': self.toggle_auto_pick_mode,
            'flood_fill': self.toggle_flood_fill_mode,
            #'draw': self.toggle_draw_mode  # Add this if you have a basic draw mode
        }
        
        for mode_name, toggle_method in modes.items():
            if mode_name != active_mode and getattr(self, f'{mode_name}_mode', False):
                toggle_method()  # This will flip the state
    ...
    def open_image(self) -> None:
        ...
        # After successful load, enable tools
        self.eraser_button.setEnabled(True)
        self.flood_fill_button.setEnabled(True)
        self.auto_pick_button.setEnabled(True)
    ...
    def finish_eraser(self) -> None:
        # Prevent operation if no image loaded
        if not hasattr(self, 'image') or self.image is None:
        ...
        for entry in self.undo_stack:
            ...
            if entry.get('type') == 'flood_fill':
                ...
                if list(updated_mask.getdata()) != list(original_mask.getdata()):
                    ...
                    overlay_item = entry.get('overlay')
                    if overlay_item is not None:
                        current_scene = overlay_item.scene()
                        if current_scene is not None and current_scene == self.scene:
    ...
    def perform_flood_fill(self, x: int, y: int):
        ...
        draw.rectangle([(0, 0), self.image.size], outline=0, width=-1)
        ...
        color_layer = Image.new("RGBA", self.image.size, self.current_color.name())
        #color_layer = Image.new("RGBA", self.image.size, QColor(self.current_color).name())
        ...
        self.undo_stack.append({
            ...
            'color': QColor(self.current_color)  # Make a copy
        })
        self.redo_stack.clear()
